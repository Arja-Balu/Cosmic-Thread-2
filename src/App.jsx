...
function App() {
  ...
  const phases = [
    {
      ...
      description:
        "NOTHING CAN ESCAPE ITS IMMENSE GRAVITATIONAL PULL. I WISH NO GOOD OPPORTUNITY EVER SLIPS AWAY FROM YOU..",
      ref: blackHoleRef,
    },
    {
      number: "02",
      ...
      description:
        "ONE OF THE DENSEST OBJECTS IN THE UNIVERSE. I WISH YOU ALWAYS REMAIN STRONG, NO MATTER HOW HEAVY LIFE GETS.",
      ref: neutronStarRef,
    },
    {
      number: "03",
      label: "WELCOME TO THE CLOUD, IT’S A 'NEBULA'.",
      description:
        "WHERE CLOUDS OF DUST AND GAS CAN BECOME THE BIRTHPLACE OF NEW STARS. I WISH YOU ALWAYS TURN EVERY BEGINNING INTO SOMETHING BEAUTIFUL.",
      ref: nebulaPhaseRef,
    },
    {
      number: "04",
      label: "SMILE FOR THE BEAM, YOU ARE NEAR A 'PULSAR'.",
      description:
        "A RAPIDLY ROTATING STAR THAT SENDS PRECISE BEAMS OF ENERGY THROUGH SPACE. I WISH YOUR LIGHT ALWAYS REACHES EVERYONE WHO NEEDS IT.",
      ref: pulsarRef,
    },
    {
      number: "05",
      ...
      description:
        "ONE OF THE BRIGHTEST OBJECTS IN THE UNIVERSE. I WISH YOU WILL ALWAYS BE THE BRIGHTEST.",
      ref: quasarRef,
    },
    {
      number: "06",
      ...
      description:
        "A DYING STAR CAN RELEASE AN ENORMOUS BURST OF ENERGY AND CREATE THE ELEMENTS THAT BUILD NEW WORLDS. I WISH EVERY GOOD ENDING IN YOUR LIFE BECOMES THE BEGINNING OF SOMETHING MORE BEAUTIFUL.",
      ref: supernovaRef,
    },
    {
      number: "07",
  ...
  const addTravelProgress = (amount) => {
    ...
    if (nextProgress >= 100) {
      ...
      // ==================================================
      // IMPORTANT FIX:
      // .cosmos has `touch-action: none` in App.css so
      // touch gestures can't scroll the page WHILE the
      // journey animation is running. That CSS rule never
      // gets lifted on its own, so on mobile the page stays
      // physically un-scrollable even after progress hits
      // 100% and overflow is unlocked above. Unlocking
      // touch-action here (inline style beats the stylesheet
      // rule) is what actually lets a finger-swipe reach the
      // Cosmic Rakhi section.
      // ==================================================

      if (cosmosRef.current) {
        cosmosRef.current.style.touchAction =
          "pan-y";
      }
    ...
    // ==================================================
    // RESPONSIVE HORIZONTAL DRIFT
    // ==================================================
    // The 220px / 45px values below were tuned against a
    // desktop-width viewport. On a narrow phone screen
    // that same fixed pixel drift eats up most of the
    // width, making everything visibly swing/lean to one
    // side ("circular table" effect) as you scroll.
    //
    // Scaling both amounts by the current viewport width
    // keeps the drift proportionally the same on every
    // screen size instead of a fixed pixel amount, so
    // objects stay centered on mobile.
    // ==================================================
    ...
    const horizontalBase =
      Math.max(
        30,
        Math.min(220, viewportWidth * 0.153)
      );

    const horizontalWiggle =
      Math.max(
        8,
        Math.min(45, viewportWidth * 0.031)
      );

    const cameraX =
      (
        progress * horizontalBase +
        Math.sin(
          progress * Math.PI * 3
        ) * horizontalWiggle
      ) * horizontalStrength;
  ...
  // ==================================================
  // COSMIC RAKHI — DOWNLOAD MESSAGE AS PDF
  // ==================================================
...
YOUR LITTLE BROTHER WILL ALWAYS BE THERE, CHEERING FOR YOU.
...
`;
  ...
  return (
    <>
      ...
      {/* ==================================================
          PAGE 2 — COSMIC RAKHI
          (Inlined directly into App so there's a single
          component/file for the whole page.)
          ================================================== */}
